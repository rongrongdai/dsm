...
layer.prompt = function(parme, yes, no){
    var log = {}, parme = parme || {}, conf = {
        ...
        dialog: {
            ...
            }() +'" class="xubox_prompt xubox_form" id="xubox_prompt" placeholder="'+(parme.placeholder||'')+'" value="'+ (parme.val || '') +'" />',
            yes: function(index){
                ...
                }else if($.isFunction(parme.validate) && !parme.validate(val)){
                    layer.tips(parme.validateMsg, '#xubox_prompt', 2);
                } else {
